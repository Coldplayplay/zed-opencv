...
int count_save_depth = 0;
int count_save_pointcloud = 0;
int count_save_image = 0;
...
void processKeyEvent(sl::Camera& zed, char &key) {
    switch (key) {
        case 'd':
        case 'D':
            saveDepth(zed, path + prefixDepth + to_string(++count_save_depth));
            break;
        ...
        case 'p':
        case 'P':
            savePointCloud(zed, path + prefixPointCloud + to_string(++count_save_pointcloud));
            break;
        ...
        case 's': // Save side by side image
            saveSbSImage(zed, std::string("left_image") 
            + std::to_string(count_save_image) + std::string(".png"),//左目存入的文件名
            std::string("right_image") 
            + std::to_string(count_save_image) + std::string(".png") );//右目存入的文件名
            
            savePointCloud(zed, std::string("cloud")+std::to_string(count_save_image));
            count_save_image++;
            break;
    }
    //count_save_image++;
...
void savePointCloud(sl::Camera& zed, std::string filename) {
    ...
    bool t = sl::savePointCloudAs(zed, PointCloud_format, filename.c_str(), true);
    if (t)
        std::cout << "Done" << endl;
    else
